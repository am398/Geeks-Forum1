...
export const CourseSearch = ({
  ...
  const onClick = ({ id, type }: { id: string, type: "channel" | "member"}) => {
    ...
    if (type === "member") {
      return router.push(`/courses/${params?.courseId}/conversations/${id}`)
    }

    if (type === "channel") {
      return router.push(`/courses/${params?.courseId}/channels/${id}`)
    }
  }
  ...
  return (
    <>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        <CommandInput placeholder="Search all channels and Learners" />
        <CommandList>
          <CommandEmpty>
            No Results found
          </CommandEmpty>
